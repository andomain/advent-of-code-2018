...
class Memory {
  ...
  run(instructionString: string): void {
  ...
  public parseInstruction(instructionString: string): Instruction {
  ...
  decimaltoBinary(input: string): string {
  ...
  maskBinaryString(binaryStr: string): string {
  ...
  maskAddress(addr: string): number[] {
    const padded = addr;
    let results: string[] = [];
